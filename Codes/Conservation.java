...
/**
 * 
 * Lengths of genes and Conservations
 * 
 */
...
public class Conservation{
  ...
  public void getSDV(int[][] starts){
    ...
    int begin = Integer.parseInt(chrome[0].split(" ")[0]);
    try{
      FileWriter writer = new FileWriter("conservationsSDV.txt"); 
      while (index < starts.length){
        if (chromeIndex != starts[index][2]){
          chromeIndex++;
          chrome = getChrome(chromes[chromeIndex] + ".txt");
  ...
  private String[] getChrome(String fileName){
    ...
    try {
      ...
      int last = 0;
      while ((s = br.readLine()) != null) {
        int temp = Integer.parseInt(s.split(" ")[0]);
        while (temp != last){
          c.add(last + " -1");
          last++;
        } 

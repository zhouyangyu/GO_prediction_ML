...
for z in range(7):
	log = KNeighborsClassifier(n_neighbors = z * 2 + 15, metric = 'manhattan', weights = 'distance')
	#log = neural_network.MLPClassifier(hidden_layer_sizes = [2 * (z + 1)], algorithm = 'sgd')
	#log = SVC(degree = 2, kernel = 'poly', C = 10 ** (-z))
	#log = neural_network.MLPClassifier(hidden_layer_sizes = [29,29], algorithm = 'adam', alpha = 0.01)
	#log = BaggingClassifier(LogisticRegression(C = 1), n_estimators = 10* (z + 1), n_jobs = -1) 
	#log = LinearSVC(C = 0.1)
	#log = RandomForestClassifier(n_estimators = 6 + z)
	#log = BaggingClassifier(LogisticRegression(C = 1), n_estimators = 10 * (z + 1), n_jobs = -1)
	#log = LogisticRegression(C = 10**(-z + 1))
	#log = AdaBoostRegressor(LogisticRegression(C = 1), n_estimators = 30)
	score = []
	tp = 0.0
	fp = 0.0
	fn = 0.0
	...
	#print(cm_normalized)
	#plt.figure()
	#plot_confusion_matrix(cm_normalized, title='Confusion matrix for Adaboost Logistic Regression(%d components)' %( 100 * (z + 1)))
